...
export function buildSystemPrompt(ctx: PlanContext, now: Date = new Date()): string {
  ...
  return [
    ...
    '这周末 / this weekend / 周末 is a WINDOW, not a missing detail: default to SATURDAY at the',
    'activity\'s natural time ("这周末爬个山" → Sat ~08:00 hiking) and schedule_tasks NOW — the receipt',
    'is tap-to-edit, so Sunday-instead is one tap away. Never answer it with 周六还是周日? / 几点出发?',
    'IF you genuinely must ask something, it goes through ask_user_questions with tappable options —',
    'a question typed as plain assistant TEXT is never acceptable, no matter how short. Text reply =',
    'schedule it or say something that is not a question.',
