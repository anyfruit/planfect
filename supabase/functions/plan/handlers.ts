...
export function buildSystemPrompt(ctx: PlanContext): string {
  ...
  return [
    ...
    'Use common sense about WHEN an activity naturally happens, and prefer free time in that window',
    'instead of just the next open gap:',
    '- library / errands / shopping / appointments / studying -> daytime, ~9:00-18:00;',
    '- workout / run / gym -> early morning ~7:00-9:00 or evening ~18:00-21:00;',
    '- a meal with someone -> lunch ~12:00-13:00 or dinner ~18:00-20:00;',
    '- focused / deep work -> mornings; relaxing / reading / social -> evenings.',
