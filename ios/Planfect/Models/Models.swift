...
struct PlanRequest: Encodable {
    ...
    var stream: Bool? = nil                    // ask for SSE instead of one buffered response
    /// Generated by us BEFORE sending, so the turn can be collected later by id even if the
    /// response never arrives (backgrounded, killed, network flipped).
    var turn_id: String? = nil
    var fetch_turn: String? = nil              // collect a previously-sent turn's saved result
}

struct PlanResponse: Decodable {
    let type: String?           // questions | scheduled | message; nil while `pending`
    let questions: [PlanQuestion]?
    let receipt: Receipt?
    let text: String?
    let messages: [JSONValue]?
    let turn_id: String?
    let pending: Bool?          // the server hasn't finished this turn yet — ask again later
}

/// One frame of a streamed turn.
enum PlanStreamEvent {
    case tool(String)      // a server-side tool started — what the turn is doing right now
    case delta(String)     // more of the visible reply
    case result(PlanResponse)
}
